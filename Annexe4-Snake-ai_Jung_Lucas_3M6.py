...
class Snake:
    ...
    def newhead(self):
        ...
        self.coords.insert(0, (self.coords[0][0] + self.direc[0], self.coords[0][1] + self.direc[
            1]))  # adds a new point depending on its actual direction and its old head
        if self.display:
    ...
    def shorten(self):
        ...
        :return: None
        """
        del self.coords[len(self.coords) - 1]  # deletes the last point of its tail
        if self.display:
            del self.show[len(self.show) - 1]  # deletes the dispaly of the last point of its tail if needed
...
class Population:
    ...
    def naturalselection(self, maxsteps):
        ...
        for i in range(len(new_pop.pop)):
            individual = self.selectindividual()  # selections a snake
        ...
        new_pop.pop[-1].brain.w1 = self.pop[-1].brain.w1.copy()  # includes the best snake of this population in the new one (without mutation)
...
class Display:
    ...
    def __init__(self, title, size, gen, maxsteps):
        ...
        :param int size: Dimension of the playable area
